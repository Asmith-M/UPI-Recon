...
def extract_txn_id(text: str) -> Optional[str]:
    ...
    """
    # Regex pattern to find prefixes followed by digits
    pattern = r"\b(?:txn\s?id|txd|txn|transaction|trans|trx)[\s:_=-]*(\d+)\b"
...
def extract_rrn(text: str) -> Optional[str]:
    ...
    Looks for optional RRN prefix followed by 12 digits.
