...
master_credits = df_master[df_master["Direction"] == "CR"].copy().reset_index(drop=True)
master_debits = df_master[df_master["Direction"] == "DR"].copy().reset_index(drop=True)

# Simulate realistic posting percentages
df_cbs_inward = master_credits.sample(frac=0.92, random_state=SEED).copy()
df_cbs_inward["Posted_Type"] = "CBS_INWARD"
df_cbs_inward["Source"] = "CBS"

df_cbs_outward = master_debits.sample(frac=0.90, random_state=SEED).copy()
df_cbs_outward["Posted_Type"] = "CBS_OUTWARD"
...
npc_in = switch_copy[switch_copy["Direction"] == "CR"].sample(frac=0.93, random_state=SEED).copy().reset_index(drop=True)
npc_out = switch_copy[switch_copy["Direction"] == "DR"].sample(frac=0.90, random_state=SEED).copy().reset_index(drop=True)
